...
public static void main(String[] args) {
    ...
    int size = scanner.nextInt();  
     ...
     for(int counter = 1; counter <= size;){
        ...
        if(newLink.checkduplicate(temp) == true){
...
class LinkedList{
    ...
    public boolean checkduplicate(int data){       //check and loop inside the list to check if there is a  duplicate
        Link current = head;
        boolean hasDuplicate = false;

        while(current != null){
            if(current.data == data){
                System.out.println("Can't Accept Duplicate Values!");
                hasDuplicate = true;
                break;
            }
    ...
    public  void sortingElements(){                           //sorting elements with the use of Arraylist
        ...
        ArrayList<Integer> setOfTwo = new ArrayList<>();
        ...
        while (ptr1.next != null) {
          ...
          while(mid.next != null){
            ...
            for (int step = 0; step < setOfTwo.size() - 1; step++) {             //sorting elements of Two Set using selection sort algorithm
                int min_idx = step; 
                for (int i = step + 1; i < setOfTwo.size(); i++) { 
                if (setOfTwo.get(i) < setOfTwo.get(min_idx)) { 
                min_idx = i; 
                } 
                } 
                int temp = setOfTwo.get(step); 
               setOfTwo.set(step, setOfTwo.get(min_idx)); 
                setOfTwo.set(min_idx, temp); 
                }
                Link first = head;
                for(int i = 0; i < setOfTwo.size(); i++){
                    first.data = setOfTwo.get(i);
                    first = first.next;
                }
                display();
